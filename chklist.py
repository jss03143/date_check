...
SHEET_NAME = 'date chklist'
FONT_NAME = "메이플스토리 Light"
# ===========================================

class MapleFinalApp:
    def __init__(self, root):
        self.root = root
        self.root.title("📊 일정 체크리스트")
        
        # 4K 환경을 고려한 기본 창 크기 설정
        self.root.geometry("1850x1000")

        # 창 종료(X) 버튼 클릭 시 실행될 프로토콜 연결
        ...
        # 창 크기 변경 시 UI 갱신 (지연 실행)
        self._after_id = None
        self.root.bind("<Configure>", self.on_window_resize)

    def on_closing(self):
        # 프로그램 종료 시 호출되어 데이터를 자동 저장함
        try:
            # 삭제/편집 모드가 아닐 때 안전하게 자동 저장 시도
    ...
    def setup_google_sheets(self):
        try:
            ...
            try:
                ...
                # 'Main'이라는 이름의 탭이 없다면 에러 메시지 출력
                messagebox.showerror("오류", "'Main' 워크시트를 찾을 수 없습니다.\n구글 시트의 탭 이름을 확인해주세요.")
                self.root.destroy()
                
        except Exception as e:
            messagebox.showerror("연결 오류", f"시트 연결 실패: {e}")
            self.root.destroy()
    ...
    def refresh_data_from_sheet(self):
        try:
            ...
            for row in all_records:
                main_cat = str(row.get('MainCategory', '기타'))
                cycle = row.get('Cycle', '주간')
                if cycle in ['weekly', '주간']: cycle = '주간'
                else: cycle = '월간'
                
                order = row.get('Order', 999)
                if main_cat not in self.data:
                    self.data[main_cat] = {"주간": [], "월간": []}
    ...
    def create_widgets(self):
        # 상단 입력부
        input_frame = tk.LabelFrame(self.root, text="🆕 새 할 일 추가", padx=10, pady=10, font=self.cat_font)
        input_frame.pack(fill="x", padx=20, pady=10)
        
        top_inner = tk.Frame(input_frame)
        top_inner.pack(pady=5)

        tk.Label(top_inner, text="📂 대주제:", font=self.large_font).grid(row=0, column=0, padx=5)
        ...
        tk.Label(top_inner, text="📝 할 일:", font=self.large_font).grid(row=0, column=2, padx=5)
        ...
        self.cycle_combo = ttk.Combobox(top_inner, values=["주간", "월간"], state="readonly", font=self.large_font, width=8)
        self.cycle_combo.current(0)
        self.cycle_combo.grid(row=0, column=4, padx=5)

        tk.Button(input_frame, text="추가하기", command=self.add_task, font=self.bold_font).pack(fill="x", pady=5)

        # 제어 버튼
        btn_frame = tk.Frame(self.root)
        btn_frame.pack(fill="x", padx=20, pady=5)
        
        tk.Button(btn_frame, text="💾 수동 저장", bg="#4CAF50", fg="white", font=self.bold_font, command=self.sync_to_sheet).pack(side="left", expand=True, fill="x", padx=2)
        self.edit_btn = tk.Button(btn_frame, text="⚙️ 편집 모드 OFF", bg="#607D8B", fg="white", font=self.bold_font, command=self.toggle_edit_mode)
        self.edit_btn.pack(side="left", expand=True, fill="x", padx=2)
        self.mode_btn = tk.Button(btn_frame, text="🗑️ 삭제 모드 OFF", bg="#9E9E9E", fg="white", font=self.bold_font, command=self.toggle_delete_mode)
    ...
    def refresh_ui(self):
        ...
        for index, main_cat in enumerate(self.cat_order):
            ...
            cat_label = tk.Label(title_bar, text=f"📂 {main_cat}", font=self.cat_font, bg=bg_color, cursor="hand2")
            cat_label.pack(side="left", padx=5, pady=5)
            cat_label.bind("<Button-1>", lambda e, c=main_cat: self.set_cat_from_click(c))

            if self.is_edit_mode:
                tk.Button(title_bar, text="▶", command=lambda i=index: self.move_category(i, 1), font=("Arial", 6), width=2).pack(side="right")
                tk.Button(title_bar, text="◀", command=lambda i=index: self.move_category(i, -1), font=("Arial", 6), width=2).pack(side="right")
            
            for cycle in ["주간", "월간"]:
                tasks = self.data[main_cat][cycle]
                if tasks:
                    tk.Label(card, text=f"• {cycle}", fg="#666666", bg=bg_color, font=self.small_font).pack(anchor="w", padx=5)
                    for i, item in enumerate(tasks):
                        ...
                        if self.is_edit_mode:
                            ...
                            tk.Button(btn_f, text="▲", command=lambda m=main_cat, cy=cycle, idx=i: self.move_task(m, cy, idx, -1), font=("Arial", 6), width=1).pack()
                            tk.Button(btn_f, text="▼", command=lambda m=main_cat, cy=cycle, idx=i: self.move_task(m, cy, idx, 1), font=("Arial", 6), width=1).pack()
    ...
    def sync_to_sheet(self, silent=False):
        ...
        try:
            ...
            if not silent: messagebox.showinfo("완료", "구글 시트 저장 성공!")
        except:
            if not silent: messagebox.showerror("오류", "시트 저장 중 오류 발생")

    def toggle_edit_mode(self):
        self.is_edit_mode = not self.is_edit_mode
        self.is_delete_mode = False
        self.edit_btn.config(text="✨ 편집 모드 ON" if self.is_edit_mode else "⚙️ 편집 모드 OFF", bg="#2196F3" if self.is_edit_mode else "#607D8B")
        self.mode_btn.config(text="🗑️ 삭제 모드 OFF", bg="#9E9E9E")
        self.refresh_ui()
    ...
    def toggle_delete_mode(self):
        ...
        if self.is_delete_mode:
            self.mode_btn.config(text="🔥 선택 항목 삭제 실행", bg="#f44336")
        else:
    ...
    def execute_delete_checked(self):
        ...
        if messagebox.askyesno("삭제 확인", f"{len(to_delete)}개 항목을 영구 삭제하시겠습니까?"):
